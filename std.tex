...
% while true; do inotifywait -e CLOSE_WRITE stdbook.tex; git add -A && git commit -m "w" && git push --set-upstream origin master; done; ///
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
$\frac{w}{w}\frac{w}{w}$
\[
wwwwwwwwwwwwwwww \sum_{n=1}^{\infty} a_n z^n \implies	\sum_{k=0}^{\infty} c_k (x-a)^k 
\]    
